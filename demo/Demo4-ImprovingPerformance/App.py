...
# Streamlit UI setup
st.title("Chat with Knowledge Base with Streaming")
...
def generate_conversation(bedrock_client, model_id, system_prompts, messages, streaming=False):
    ...
    if streaming:
        ...
        if stream:
            ...
            for event in stream:
                ...
                if 'contentBlockDelta' in event:
                    ...
                    last_response_time = current_time - start_time
        ...
        return full_text, input_tokens, output_tokens, first_response_time, last_response_time
        ...
        return output_text, input_tokens, output_tokens, response_time, response_time
...
if st.button("Generate"):
    ...
    try:
        output_text, input_tokens, output_tokens, first_response_time, last_response_time = generate_conversation(
            client, model_id, system_prompts, messages, streaming
        )
        ...
        # Display response time information
        st.subheader("Response Time Information")
        if streaming:
            st.write(f"Time to first token: {first_response_time:.2f} seconds")
            st.write(f"Total response time: {last_response_time:.2f} seconds")
        else:
            st.write(f"Response time: {first_response_time:.2f} seconds")
