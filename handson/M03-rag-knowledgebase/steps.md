...
2. ベクトルストアが「S3 Vectors」であることを確認
3. テスト画面で質問を入力し、検索結果と生成回答を表示
4. 検索されたチャンクとスコアを確認
5. 「もっと詳しく」と続けて会話の継続を実演

### デモ 2: チャンキング戦略の比較（5分）
1. `python3.12 chunking_optimization.py` を実行
2. 固定サイズ: 文が途中で切れる問題を表示
3. 階層型: セクション単位で完全なコンテキストが得られることを確認
4. 検索精度スコアの違いを数値で比較

### デモ 3: RAGAS 評価（5分）
1. `python3.12 rag_evaluation.py` を実行
2. 4指標（忠実性、関連性、精度、再現率）の意味を解説
3. スコアに基づく改善提案を確認
4. 改善サイクル（測定→分析→改善→再測定）を説明

### デモ 4: 会話型アシスタント（5分）
1. `python3.12 knowledge_assistant.py` を実行
2. マルチターン会話で文脈保持を実演
3. アクセス制御でロール別の回答の違いを表示
4. 「この質問は範囲外です」とスコープ制限が作動する例

### デモ 5: ハイブリッド検索（OpenSearch 側で実施）（5分）
1. `cd ~/handson/M03-opensearch-vectorsearch`
2. `python3.12 hybrid_search.py --demo` を実行
3. キーワード検索 / セマンティック検索 / ハイブリッド検索の結果を比較
4. S3 Vectors と OpenSearch の使い分けを解説
...
1. **RAGAS 継続評価パイプライン**: CloudWatch + Lambda で定期的に RAGAS 評価を実行し、品質低下を検知
2. **多言語対応**: 日本語と英語の混在ドキュメントでの検索精度向上
3. **リアルタイム更新**: 新規ドキュメント追加時の EventBridge → Lambda → 自動 Sync パイプライン
4. **Re-Ranking 統合**: LLM Re-Ranking をプロダクション RAG パイプラインに組み込み
5. **チャンキング A/B テスト**: 同一クエリセットで複数設定を並行評価し、最適設定を自動選択
